function main() {
  function getValue(input) {
    const logs = input.split('\n').map(line => {
      const { date, minute, guardId, event } = line.match(
        /^\[(?<date>\d{4}-\d{2}-\d{2} \d{2}:(?<minute>\d{2}))\] (Guard #(?<guardId>\d+) |)(?<event>.*?)$/
      ).groups;
      return {
        date,
        minute: parseInt(minute, 10),
        guardId: guardId ? parseInt(guardId, 10) : undefined,
        event,
      };
    });
    logs.sort((a, b) => {
    ...
    const minuteSleepFrequenciesByGuard = new Map();

    let currentGuardId = 0;
    let fellAsleepAtMinute = 0;
    logs.forEach(log => {
      if (log.event === 'begins shift') {
        currentGuardId = log.guardId;
      }
      if (log.event === 'falls asleep') {
        if (!minuteSleepFrequenciesByGuard.has(currentGuardId)) {
          minuteSleepFrequenciesByGuard.set(currentGuardId, new Map());
        }
        const previousFrequency =
          minuteSleepFrequenciesByGuard.get(currentGuardId).get(log.minute) ||
          0;
        minuteSleepFrequenciesByGuard
          .get(currentGuardId)
          .set(log.minute, previousFrequency + 1);
        fellAsleepAtMinute = log.minute;
      }
      if (log.event === 'wakes up') {
        for (
          let minute = fellAsleepAtMinute + 1;
          minute < log.minute;
          minute++
        ) {
          const previousFrequency =
            minuteSleepFrequenciesByGuard.get(currentGuardId).get(minute) || 0;
          minuteSleepFrequenciesByGuard
            .get(currentGuardId)
            .set(minute, previousFrequency + 1);
        }
      }
    });
    ...
    let bestMinute = 0;
    let bestGuardId = 0;
    minuteSleepFrequenciesByGuard.forEach((minuteSleepFrequencies, guardId) => {
      let bestMinuteForGuard = 0;
      let maxFrequencyForGuard = 0;
      minuteSleepFrequencies.forEach((frequency, minute) => {
        if (frequency > maxFrequencyForGuard) {
          maxFrequencyForGuard = frequency;
          bestMinuteForGuard = minute;
        }
      });

      if (maxFrequencyForGuard > maxFrequency) {
        maxFrequency = maxFrequencyForGuard;
        bestMinute = bestMinuteForGuard;
        bestGuardId = guardId;
      }
    });
...
[1518-11-05 00:55] wakes up
  ...
  console.log('Result:', getValue(givenInput));
}
