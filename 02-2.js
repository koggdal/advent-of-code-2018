function main() {
  function getCommonLettersFromCorrectBoxes(input) {
    const boxIds = input.split('\n');

    for (let id of boxIds) {
      const mismatches = {};
      let filteredBoxIds = boxIds;

      [...id].forEach((letter, index) => {
        filteredBoxIds = filteredBoxIds.filter(otherId => {
          if (otherId[index] !== letter) {
            if (!(otherId in mismatches)) {
              mismatches[otherId] = 0;
            }
            mismatches[otherId]++;
            if (mismatches[otherId] > 1) {
              return false;
            }
          }
          return true;
      ...
      if (filteredBoxIds.length === 2) {
        return [...filteredBoxIds[0]].reduce((commonLetters, letter, index) => {
          return (
            commonLetters + (letter === filteredBoxIds[1][index] ? letter : '')
          );
        }, '');
      }
    }
...
wvxyz
  ...
  console.log('Result:', getCommonLettersFromCorrectBoxes(givenInput));
}
